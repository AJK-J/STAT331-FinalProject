...
p <- all_data |>
  ggplot() +
  aes(x = completion_rate, y = vacc_rate, size = 8,color = country) +
...
```
We animated a plot of the relationship between primary school completion rate and vaccination rate, with each frame being a new year to show how the relationship changes over time. Each new set of points leaves a marker to show the graph being populated as data was collected.
...
Animation over the years for the plot of year vs both primary school completion rate and vaccination rate. Markers are left from each year as the animation moves to the next year so that the praph populates over time.
